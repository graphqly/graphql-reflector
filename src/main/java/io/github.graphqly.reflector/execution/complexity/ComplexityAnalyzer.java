...
/**
 * Class used to perform static complexity analysis on the parsed operation AST. It recursively
 * walks the AST and accumulates the complexity scores. Once the threshold is exceeded, it throws a
 * {@link ComplexityLimitExceededException}. The complexity score calculation for each node is
 * delegated to {@link ComplexityFunction}.
 */
class ComplexityAnalyzer {
  ...
  ResolvedField collectFields(ExecutionContext context) {
    FieldCollectorParameters parameters =
        FieldCollectorParameters.newParameters()
            ...
            .build();
    List<Field> fields =
        context.getOperationDefinition().getSelectionSet().getSelections().stream()
    ...
    Map<String, Object> argumentValues =
        valuesResolver.getArgumentValues(
            fieldDefinition.getArguments(), field.getArguments(), context.getVariables());
    return collectFields(
        parameters,
        fields.stream()
            .map(f -> new ResolvedField(f, fieldDefinition, argumentValues))
            .collect(Collectors.toList()));
  }
   ...
   */
  private Map<String, ResolvedField> collectFields(
      FieldCollectorParameters parameters, List<Field> fields, GraphQLFieldsContainer parent) {
    ...
    fields.stream()
        ...
        .forEach(
            field ->
                collectFields(
                    parameters,
                    unconditionalSubFields,
                    getUnconditionalSelections(field.getSelectionSet()),
                    visitedFragments,
                    parent));
    ...
    fields.stream()
        ...
        .forEach(
            field ->
                getConditionalSelections(field.getSelectionSet())
                    .forEach(
                        (condition, selections) -> {
                          Map<String, List<ResolvedField>> subFields = new LinkedHashMap<>();
                          collectFields(
                              parameters, subFields, selections, visitedFragments, parent);
  ...
  private ResolvedField collectFields(
      FieldCollectorParameters parameters, List<ResolvedField> fields) {
    ...
    List<Field> rawFields =
        fields.stream().map(ResolvedField::getField).collect(Collectors.toList());
    Map<String, ResolvedField> children =
        collectFields(parameters, rawFields, (GraphQLFieldsContainer) field.getFieldType());
    ResolvedField node =
        new ResolvedField(
            field.getField(), field.getFieldDefinition(), field.getArguments(), children);
  ...
  private void collectFields(
      FieldCollectorParameters parameters,
      Map<String, List<ResolvedField>> fields,
    ...
    for (Selection selection : selectionSet) {
      if (selection instanceof Field) {
        ...
        collectInlineFragment(
            parameters, fields, visitedFragments, (InlineFragment) selection, parent);
      } else if (selection instanceof FragmentSpread) {
        collectFragmentSpread(
            parameters, fields, visitedFragments, (FragmentSpread) selection, parent);
      }
    }
  }
  ...
  private void collectFragmentSpread(
    ...
    if (!conditionalNodes.shouldInclude(
        parameters.getVariables(), fragmentSpread.getDirectives())) {
      return;
    }
    visitedFragments.add(fragmentSpread.getName());
    FragmentDefinition fragmentDefinition =
        parameters.getFragmentsByName().get(fragmentSpread.getName());

    if (!conditionalNodes.shouldInclude(
        parameters.getVariables(), fragmentDefinition.getDirectives())) {
      return;
    }
    if (fragmentDefinition.getTypeCondition() != null) {
      parent =
          (GraphQLFieldsContainer)
              getTypeFromAST(parameters.getGraphQLSchema(), fragmentDefinition.getTypeCondition());
    }
    collectFields(
        parameters,
        fields,
  ...
  private void collectInlineFragment(
      FieldCollectorParameters parameters,
      Map<String, List<ResolvedField>> fields,
      List<String> visitedFragments,
    ...
    if (!conditionalNodes.shouldInclude(
        parameters.getVariables(), inlineFragment.getDirectives())) {
      return;
    }
    if (inlineFragment.getTypeCondition() != null) {
      parent =
          (GraphQLFieldsContainer)
              getTypeFromAST(parameters.getGraphQLSchema(), inlineFragment.getTypeCondition());
    }
    collectFields(
        parameters,
        fields,
        inlineFragment.getSelectionSet().getSelections(),
        visitedFragments,
        parent);
  }
  ...
  private void collectField(
    ...
    Map<String, Object> argumentValues =
        valuesResolver.getArgumentValues(
            fieldDefinition.getArguments(), field.getArguments(), parameters.getVariables());
  ...
  protected Map<String, ResolvedField> reduceAlternatives(
      FieldCollectorParameters parameters,
    ...
    for (Map.Entry<String, Map<String, List<ResolvedField>>> conditional :
        conditionalSubFields.entrySet()) {
      Map<String, List<ResolvedField>> merged = new HashMap<>(conditional.getValue());
      for (Map.Entry<String, List<ResolvedField>> unconditional :
          unconditionalSubFields.entrySet()) {
        merged.merge(
            unconditional.getKey(),
            unconditional.getValue(),
            (condNodes, uncondNodes) ->
                Stream.concat(condNodes.stream(), uncondNodes.stream())
                    .collect(Collectors.toList()));
      }
      Map<String, ResolvedField> flat =
  ...
  private boolean isConditional(Selection selection) {
    return (selection instanceof FragmentDefinition
            && ((FragmentDefinition) selection).getTypeCondition() != null)
        || (selection instanceof InlineFragment
            && ((InlineFragment) selection).getTypeCondition() != null);
  }

  private GraphQLObjectType getRootType(
      GraphQLSchema schema, OperationDefinition operationDefinition) {
    if (operationDefinition.getOperation() == OperationDefinition.Operation.MUTATION) {
      ...
      throw new IllegalStateException(
          "Unknown operation type encountered. Incompatible github.io.github.graphqly.reflector.reflector-java version?");
    }
  }
}
