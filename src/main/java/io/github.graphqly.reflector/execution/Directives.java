...
public class Directives {
  ...
  Directives(DataFetchingEnvironment env, ExecutionStepInfo step) {
    ...
    fields.forEach(
        field ->
            directives.merge(
                ...
                parseDirectives(field.getDirectives(), env),
                (directiveMap1, directiveMap2) -> {
                  directiveMap2.forEach(
                      (directiveName, directiveValues) ->
                          directiveMap1.merge(
                              directiveName,
                              directiveValues,
                              (valueList1, valueList2) ->
                                  Stream.concat(valueList1.stream(), valueList2.stream())
                                      .collect(Collectors.toList())));
                  return directiveMap1;
                }));

    // Operation directives
    Map<String, List<Map<String, Object>>> operationDirectives =
        parseDirectives(env.getOperationDefinition().getDirectives(), env);
    if (OperationDefinition.Operation.MUTATION.equals(
        env.getOperationDefinition().getOperation())) {
    ...
    if (step.hasParent() && step.getParent().getField() != null) {
      FragmentDirectiveCollector fragmentDirectiveCollector =
          FragmentDirectiveCollector.collect(env, step);
      directives.put(
  ...
  private Map<String, List<Map<String, Object>>> parseDirectives(
      List<Directive> directives, DataFetchingEnvironment env) {
  ...
  public List<Map<String, Object>> find(
      Introspection.DirectiveLocation location, String directiveName) {
  ...
  private static class FragmentDirectiveCollector extends QueryVisitorStub {
    ...
    public static FragmentDirectiveCollector collect(
        DataFetchingEnvironment env, ExecutionStepInfo step) {
      FragmentDirectiveCollector fragmentDirectiveCollector = new FragmentDirectiveCollector(env);
      // This is safe because top-level fields don't get to here and all deeper fields at least have
      // a parent (source object) and a grand-parent (query root)
    ...
    public void visitInlineFragment(QueryVisitorInlineFragmentEnvironment env) {
      ...
      boolean isRelevant =
          containsField
              || relevantFragments.stream()
                  .anyMatch(frag -> fragment.getSelectionSet().getSelections().contains(frag));
      if (isRelevant) {
