...
public interface SimpleModule extends Module {
  ...
  default void setUp(SetupContext context) {
    if (!getResolverBuilders().isEmpty()) {
      context
          .getSchemaGenerator()
          .withResolverBuilders(getResolverBuilders().toArray(new ResolverBuilder[0]));
    }
    ...
    if (!getOutputConverters().isEmpty()) {
      context
          .getSchemaGenerator()
          .withOutputConverters(getOutputConverters().toArray(new OutputConverter[0]));
    }
    if (!getInputConverters().isEmpty()) {
      context
          .getSchemaGenerator()
          .withInputConverters(getInputConverters().toArray(new InputConverter[0]));
    }
    if (!getArgumentInjectors().isEmpty()) {
      context
          .getSchemaGenerator()
          .withArgumentInjectors(getArgumentInjectors().toArray(new ArgumentInjector[0]));
    }
