...
public class JsonArrayAdapter extends AbstractTypeSubstitutingMapper<List<JsonNode>>
    implements InputConverter<ArrayNode, List<JsonNode>>,
        DelegatingOutputConverter<ArrayNode, List> {

  private static final AnnotatedType JSON = GenericTypeReflector.annotate(JsonNode.class);

  @Override
  public ArrayNode convertInput(
      List<JsonNode> substitute,
      AnnotatedType type,
  ...
  public AnnotatedType getSubstituteType(AnnotatedType original) {
    return GenericTypeReflector.annotate(
        new TypeToken<List<JsonNode>>() {}.getType(), original.getAnnotations());
  }

  @Override
  public List convertOutput(
      ArrayNode original, AnnotatedType type, ResolutionEnvironment resolutionEnvironment) {
