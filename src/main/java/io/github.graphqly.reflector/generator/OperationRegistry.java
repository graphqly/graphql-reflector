...
import io.github.graphqly.reflector.metadata.strategy.query.ResolverBuilderParams;
...
public class OperationRegistry {
  ...
  public OperationRegistry(
      OperationSourceRegistry operationSourceRegistry,
    ...
    List<Resolver> mutationResolvers =
        buildMutationResolvers(operationSourceRegistry.getOperationSources());
    List<Resolver> subscriptionResolvers =
        buildSubscriptionResolvers(operationSourceRegistry.getOperationSources());
  ...
  private Set<Operation> buildQueries(List<Resolver> resolvers) {
    return resolvers.stream()
        .collect(Collectors.groupingBy(Resolver::getOperationName))
        .values()
        .stream()
        .flatMap(
            r ->
                collectContextTypes(r).stream()
                    .map(contextType -> resolversPerContext(contextType, r))
                    .filter(contextual -> !contextual.getValue().isEmpty())
                    .map(
                        contextual ->
                            operationBuilder.buildQuery(
                                contextual.getKey(), contextual.getValue(), environment)))
        ...
        .flatMap(
            r ->
                collectContextTypes(r).stream()
                    ...
                    .map(
                        contextual ->
                            operationBuilder.buildMutation(
                                contextual.getKey(), contextual.getValue(), environment)))
        .collect(Collectors.toCollection(LinkedHashSet::new));
  }
        ...
        .flatMap(
            r ->
                collectContextTypes(r).stream()
                    ...
                    .map(
                        contextual ->
                            operationBuilder.buildSubscription(
                                contextual.getKey(), contextual.getValue(), environment)))
        .collect(Collectors.toCollection(LinkedHashSet::new));
  }

  private Map.Entry<Type, List<Resolver>> resolversPerContext(
      Type context, List<Resolver> resolvers) {
    List<Resolver> contextual;
    if (context == null) {
      contextual =
          resolvers.stream().filter(r -> r.getSourceTypes().isEmpty()).collect(Collectors.toList());
    } else {
      contextual =
          resolvers.stream()
              .filter(r -> r.getSourceTypes().contains(context))
  ...
  public Collection<Operation> getChildQueries(AnnotatedType domainType) {
    ...
    Map<String, Operation> nestedQueries =
        getNestedQueries(domainType).stream()
    ...
    Map<String, Operation> embeddableQueries =
        getEmbeddableQueries(domainType.getType()).stream()
            .collect(Collectors.toMap(Operation::getName, Function.identity()));
  ...
  private List<Resolver> buildQueryResolvers(Collection<OperationSource> operationSources) {
    return buildResolvers(
        operationSources,
        ((operationSource, builder) ->
            builder.buildQueryResolvers(
                new ResolverBuilderParams(
                    operationSource.getServiceBeanSupplier(),
                    operationSource.getJavaType(),
                    inclusionStrategy,
                    typeTransformer,
                    basePackages,
                    environment))));
  ...
  private List<Resolver> buildMutationResolvers(Collection<OperationSource> operationSources) {
    return buildResolvers(
        operationSources,
        ((operationSource, builder) ->
            builder.buildMutationResolvers(
                new ResolverBuilderParams(
                    operationSource.getServiceBeanSupplier(),
                    operationSource.getJavaType(),
                    inclusionStrategy,
  ...
  private List<Resolver> buildSubscriptionResolvers(Collection<OperationSource> operationSources) {
    return buildResolvers(
        operationSources,
        ((operationSource, builder) ->
            builder.buildSubscriptionResolvers(
                new ResolverBuilderParams(
                    operationSource.getServiceBeanSupplier(),
                    operationSource.getJavaType(),
                    inclusionStrategy,
  ...
  private List<Resolver> buildResolvers(
      Collection<OperationSource> operationSources,
