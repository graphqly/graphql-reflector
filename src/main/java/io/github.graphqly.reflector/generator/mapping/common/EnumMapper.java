...
public class EnumMapper extends CachingMapper<GraphQLEnumType, GraphQLEnumType> {
  ...
  @Override
  public GraphQLEnumType toGraphQLType(
      String typeName,
      AnnotatedType javaType,
      OperationMapper operationMapper,
      BuildContext buildContext) {
    GraphQLEnumType.Builder enumBuilder =
        newEnum()
            .name(typeName)
            .description(
    ...
    buildContext
        .directiveBuilder
        .buildEnumTypeDirectives(javaType, buildContext.directiveBuilderParams())
        .forEach(
            directive ->
                enumBuilder.withDirective(
  ...
  @Override
  public GraphQLEnumType toGraphQLInputType(
      String typeName,
      AnnotatedType javaType,
      OperationMapper operationMapper,
      BuildContext buildContext) {
            ...
            enumConst ->
                enumBuilder.value(
                    GraphQLEnumValueDefinition.newEnumValueDefinition()
                        ...
                        .withDirectives(
                            getValueDirectives(enumConst, operationMapper, buildContext))
                        .build()));
  }
  ...
  protected String getValueName(Enum<?> value, MessageBundle messageBundle) {
    GraphQLEnumValue annotation =
        ClassUtils.getEnumConstantField(value).getAnnotation(GraphQLEnumValue.class);
    return annotation != null && !annotation.name().isEmpty()
        ? messageBundle.interpolate(annotation.name())
        : value.name();
  }
  ...
  protected String getValueDeprecationReason(Enum<?> value, MessageBundle messageBundle) {
    ...
    return javaDeprecationConfig.enabled && deprecated != null
        ? javaDeprecationConfig.deprecationReason
        : null;
  }

  protected GraphQLDirective[] getValueDirectives(
      Enum<?> value, OperationMapper operationMapper, BuildContext buildContext) {
    return buildContext.directiveBuilder
        .buildEnumValueDirectives(value, buildContext.directiveBuilderParams()).stream()
