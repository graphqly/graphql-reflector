...
public class ArrayAdapter extends AbstractTypeSubstitutingMapper
    implements DelegatingOutputConverter {
  ...
  public AnnotatedType getSubstituteType(AnnotatedType original) {
    ...
    if (raw.isPrimitive()) {
      component =
          GenericTypeReflector.annotate(GenericTypeReflector.box(raw), component.getAnnotations());
    }
    return TypeFactory.parameterizedAnnotatedClass(
        List.class, original.getAnnotations(), component);
  }
  ...
  public Object convertOutput(
    ...
    return IntStream.range(0, Array.getLength(original))
        .mapToObj(
            i -> resolutionEnvironment.convertOutput(Array.get(original, i), getElementType(type)))
        .toArray();
  }
