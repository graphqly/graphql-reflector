...
public class InterfaceMapper extends CachingMapper<GraphQLInterfaceType, GraphQLInputObjectType> {
  ...
  public InterfaceMapper(
      InterfaceMappingStrategy interfaceStrategy, ObjectTypeMapper objectTypeMapper) {
  ...
  @Override
  public GraphQLInterfaceType toGraphQLType(
      String typeName,
    ...
    List<GraphQLFieldDefinition> fields =
        objectTypeMapper.getFields(typeName, javaType, buildContext, operationMapper);
    fields.forEach(typeBuilder::field);

    typeBuilder.withDirective(Directives.mappedType(javaType));
    buildContext
        .directiveBuilder
        .buildInterfaceTypeDirectives(javaType, buildContext.directiveBuilderParams())
        .forEach(
            directive ->
  ...
  private void registerImplementations(
    ...
    buildContext
        ...
        .findImplementations(
            javaType,
            isImplementationAutoDiscoveryEnabled(javaType),
            getScanPackages(javaType),
            buildContext)
        .forEach(
            impl ->
                getImplementingType(impl, operationMapper, buildContext)
                    .ifPresent(
                        implType ->
                            buildContext.typeRegistry.registerDiscoveredCovariantType(
                                type.getName(), impl, implType)));
  }

  @SuppressWarnings("WeakerAccess")
  protected boolean isImplementationAutoDiscoveryEnabled(AnnotatedType javaType) {
    return javaType.isAnnotationPresent(GraphQLInterface.class)
        && javaType.getAnnotation(GraphQLInterface.class).implementationAutoDiscovery();
  }

  @SuppressWarnings("WeakerAccess")
  protected String[] getScanPackages(AnnotatedType javaType) {
    return javaType.isAnnotationPresent(GraphQLInterface.class)
        ? javaType.getAnnotation(GraphQLInterface.class).scanPackages()
        : Utils.emptyArray();
  }

  private Optional<GraphQLObjectType> getImplementingType(
      AnnotatedType implType, OperationMapper operationMapper, BuildContext buildContext) {
