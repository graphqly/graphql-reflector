 ...
 * This adapter turns a map into a list of key-value pairs (instances of {@link
 * MapToListTypeAdapter.MapEntry}).
 */
public class MapToListTypeAdapter
    extends AbstractTypeAdapter<Map<?, ?>, List<MapToListTypeAdapter.MapEntry<?, ?>>>
  ...
  @Override
  public List<MapToListTypeAdapter.MapEntry<?, ?>> convertOutput(
      Map<?, ?> original, AnnotatedType type, ResolutionEnvironment resolutionEnvironment) {
  ...
  @Override
  public Map<?, ?> convertInput(
      List<MapEntry<?, ?>> original,
      AnnotatedType type,
      GlobalEnvironment environment,
      ValueMapper valueMapper) {
    Map<Object, Object> initial =
        ClassUtils.isSuperClass(type, HashMap.class) ? new HashMap<>() : ClassUtils.instance(type);
  ...
  public AnnotatedType getSubstituteType(AnnotatedType original) {
    ...
    Type entryType =
        TypeFactory.parameterizedClass(
            MapToListTypeAdapter.MapEntry.class, keyType.getType(), valueType.getType());
    return GenericTypeReflector.annotate(
        TypeFactory.parameterizedClass(List.class, entryType), original.getAnnotations());
  }
    ...
    return new GraphQLList(
        new GraphQLNonNull(
            mapEntry(
                ...
                operationMapper.toGraphQLType(getElementType(javaType, 1), buildContext),
                buildContext)));
  }
    ...
    return new GraphQLList(
        new GraphQLNonNull(
            mapEntry(
                ...
                operationMapper.toGraphQLInputType(getElementType(javaType, 1), buildContext),
                buildContext)));
  }
  ...
  public boolean supports(AnnotatedType type) {
    return ClassUtils.isSuperClass(Map.class, type)
        && !type.isAnnotationPresent(GraphQLScalar.class);
  }

  private GraphQLOutputType mapEntry(
      GraphQLOutputType keyType, GraphQLOutputType valueType, BuildContext buildContext) {
  ...
  private GraphQLInputType mapEntry(
      GraphQLInputType keyType, GraphQLInputType valueType, BuildContext buildContext) {
    ...
    return newInputObject()
        ...
        .field(newInputObjectField().name("key").description("Map key input").type(keyType).build())
        .field(
            newInputObjectField()
                .name("value")
