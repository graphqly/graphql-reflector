...
public class EnumMapToObjectTypeAdapter<E extends Enum<E>, V>
  ...
  protected GraphQLObjectType toGraphQLType(
    ...
    Enum<E>[] keys =
        ClassUtils.<E>getRawType(getElementType(javaType, 0).getType()).getEnumConstants();
    Arrays.stream(keys)
        .forEach(
            enumValue ->
                builder.field(
                    GraphQLFieldDefinition.newFieldDefinition()
                        ...
                        .description(
                            enumMapper.getValueDescription(enumValue, buildContext.messageBundle))
                        .deprecate(
                            enumMapper.getValueDeprecationReason(
                                enumValue, buildContext.messageBundle))
                        .type(
                            operationMapper.toGraphQLType(
                                getElementType(javaType, 1), buildContext))
  ...
  protected GraphQLInputObjectType toGraphQLInputType(
    ...
    Enum[] keys =
        (Enum[]) ClassUtils.getRawType(getElementType(javaType, 0).getType()).getEnumConstants();
    Arrays.stream(keys)
        .forEach(
            enumValue ->
                builder.field(
                    GraphQLInputObjectField.newInputObjectField()
                        ...
                        .description(
                            enumMapper.getValueDescription(enumValue, buildContext.messageBundle))
                        .type(
                            operationMapper.toGraphQLInputType(
                                getElementType(javaType, 1), buildContext))
                        .build()));
    return builder.build();
  }
  ...
  @Override
  public EnumMap<E, V> convertInput(
      Map<String, V> substitute,
      AnnotatedType type,
      GlobalEnvironment environment,
      ValueMapper valueMapper) {
    Map<String, E> values =
        Arrays.stream(
                (ClassUtils.<E>getRawType(getElementType(type, 0).getType()).getEnumConstants()))
            .collect(
                Collectors.toMap(
                    e -> (enumMapper.getValueName(e, environment.messageBundle)),
                    Function.identity()));
    Map<E, V> m =
        substitute.entrySet().stream()
  ...
  public AnnotatedType getSubstituteType(AnnotatedType original) {
    ...
    return TypeFactory.parameterizedAnnotatedClass(
        ...
        GenericTypeReflector.annotate(String.class, keyType.getAnnotations()),
        valueType);
  }
