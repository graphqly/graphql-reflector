...
public class ConverterRegistry {
  ...
  public ConverterRegistry(
      List<InputConverter> inputConverters, List<OutputConverter> outputConverters) {
  ...
  public <T, S> InputConverter<T, S> getInputConverter(AnnotatedType inputType) {
    return (InputConverter<T, S>)
        inputConverters.stream().filter(conv -> conv.supports(inputType)).findFirst().orElse(null);
  }

  @SuppressWarnings("unchecked")
  public <T, S> OutputConverter<T, S> getOutputConverter(AnnotatedType outputType) {
    return (OutputConverter<T, S>)
        outputConverters.stream()
            .filter(conv -> conv.supports(outputType))
            .findFirst()
            .orElse(null);
  }
  ...
  public ConverterRegistry optimize(List<AnnotatedType> types) {
    ...
    if (filtered.stream()
        .allMatch(
            converter ->
                converter instanceof DelegatingOutputConverter
  ...
  private void collectConverters(AnnotatedType type, Set<OutputConverter> filtered) {
    ...
    if (converter != null) {
      ...
      if (converter instanceof DelegatingOutputConverter) {
        ((DelegatingOutputConverter<?, ?>) converter)
