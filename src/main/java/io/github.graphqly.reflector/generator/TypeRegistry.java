...
public class TypeRegistry {
  ...
  private final Map<String, Map<String, MappedType>> covariantOutputTypes =
                    ...
                    .forEach(
                        inter ->
                            registerCovariantType(
                                inter.getName(), Directives.getMappedType(obj), obj)));
        ...
        .forEach(
            union ->
                union.getTypes().stream()
                    .filter(
                        type -> type instanceof GraphQLObjectType && Directives.isMappedType(type))
                    .map(type -> (GraphQLObjectType) type)
                    .forEach(
                        obj ->
                            registerCovariantType(
                                union.getName(), Directives.getMappedType(obj), obj)));
  }

  public void registerDiscoveredCovariantType(
      String compositeTypeName, AnnotatedType javaSubType, GraphQLObjectType subType) {
    this.discoveredTypes.add(subType);
    registerCovariantType(compositeTypeName, javaSubType, subType);
  }

  public void registerCovariantType(
      String compositeTypeName, AnnotatedType javaSubType, GraphQLOutputType subType) {
    ...
    if (subType instanceof GraphQLObjectType
        || covariantTypes.get(subType.getName()) == null
        ...
        if (entry.getValue().graphQLType instanceof GraphQLTypeReference) {
          ...
          if (resolvedType != null) {
            ...
            log.warn(
                "Type reference "
                    + entry.getKey()
                    + " could not be replaced correctly. "
            ...
            // the edge case is when the primary resolver returns an interface or a union and not
            // the node type directly
            toRemove.add(entry.getKey());
          }
        }
      }
      toRemove.forEach(covariantTypes::remove);
      covariantTypes.replaceAll(
          (typeName, mapped) ->
              mapped.graphQLType instanceof GraphQLTypeReference
