...
public class Validator {
    ...
    this.mappedTypes =
        knownTypes.stream()
            ...
            .collect(
                Collectors.toMap(
                    GraphQLType::getName,
                    type -> ClassUtils.normalize(Directives.getMappedType(type))));
  }

  public ValidationResult checkUniqueness(GraphQLOutputType graphQLType, AnnotatedType javaType) {
    return checkUniqueness(graphQLType, () -> mappers.getMappableType(javaType));
  }

  public ValidationResult checkUniqueness(GraphQLInputType graphQLType, AnnotatedType javaType) {
  ...
  public ValidationResult checkUniqueness(
      GraphQLType graphQLType, Supplier<AnnotatedType> javaType) {
    ...
    try {
      ...
      return ValidationResult.invalid(
          String.format(
              "Exception while checking the name uniqueness for %s: %s",
              graphQLType.getName(), e.getMessage()));
    }
    ...
    return ValidationResult.invalid(
        String.format(
            "Potential type name collision detected: '%s' bound to multiple types:"
                ...
                + " If this warning is a false positive, please report it: %s.",
            graphQLType.getName(),
            knownType,
            getLoaderName(knownType),
            resolvedType,
            getLoaderName(resolvedType),
            TypeInfoGenerator.class.getSimpleName(),
