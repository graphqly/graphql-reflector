...
public class AstPrinter {
  ...
  private static void printImpl(Writer writer, Node node, boolean compactMode) {
    ...
    printer.print(new CodePrinter(writer), node);
  }
  ...
  private NodePrinter<Field> field() {
    ...
    return (out, node) -> {
      ...
      out.printf("%s", spaced(alias + name + arguments, directives, selectionSet));
    };
  }
  ...
  private NodePrinter<InlineFragment> inlineFragment() {
    return (out, node) -> {
      ...
      out.printf("%s", comments(node));
      out.printf("%s", spaced("...", typeCondition, directives, selectionSet));
    };
  }
  ...
  private NodePrinter<ObjectField> objectField() {
    ...
    return (out, node) -> {
      ...
      out.printf("%s%s%s", name, nameValueSep, value(node.getValue()));
    };
  }
  ...
  private NodePrinter<OperationTypeDefinition> operationTypeDefinition() {
    String nameTypeSep = compactMode ? ":" : ": ";
    return (out, node) ->
        out.printf("%s%s%s", node.getName(), nameTypeSep, type(node.getTypeName()));
  }
  ...
  private NodePrinter<ScalarTypeDefinition> scalarTypeDefinition() {
    return (out, node) -> {
      out.printf("%s", comments(node));
      out.printf("%s", spaced("scalar", node.getName(), directives(node.getDirectives())));
    };
  }
  ...
  private String node(Node node, Class startClass) {
    ...
    StringWriter sw = new StringWriter();
    CodePrinter out = new CodePrinter(sw);
  ...
  private <T extends Node> NodePrinter<T> _findPrinter(Node node, Class startClass) {
    ...
    throw new AssertException(
        String.format("We have a missing printer implementation for %s : report a bug!", clazz));
  }
  ...
  private interface NodePrinter<T extends Node> {
    void print(CodePrinter out, T node);
  }
}
