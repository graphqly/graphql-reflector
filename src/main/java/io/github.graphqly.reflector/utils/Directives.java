...
public class Directives {
  ...
  private static final GraphQLScalarType UNREPRESENTABLE =
  ...
  public static boolean isMappedType(GraphQLType type) {
    return type instanceof GraphQLDirectiveContainer
        && DirectivesUtil.directiveWithArg(
                ((GraphQLDirectiveContainer) type).getDirectives(), MAPPED_TYPE, TYPE)
            .isPresent();
  }

  public static AnnotatedType getMappedType(GraphQLType type) {
    return DirectivesUtil.directiveWithArg(
            ((GraphQLDirectiveContainer) type).getDirectives(), MAPPED_TYPE, TYPE)
