...
public class Operation {
  ...
  public Operation(
    ...
    if (!(resolvers.stream().allMatch(Resolver::isBatched)
        || resolvers.stream().noneMatch(Resolver::isBatched))) {
      throw new IllegalArgumentException(
          "Operation \"" + name + "\" mixes regular and batched resolvers");
    }

    this.name = name;
    this.description =
        resolvers.stream()
            .map(Resolver::getOperationDescription)
            .filter(Utils::isNotEmpty)
            .findFirst()
            .orElse(null);
    this.deprecationReason =
        resolvers.stream()
            .map(Resolver::getOperationDeprecationReason)
  ...
  private Map<String, Resolver> collectResolversByFingerprint(List<Resolver> resolvers) {
    ...
    resolvers.forEach(
        resolver -> resolversByFingerprint.putIfAbsent(resolver.getFingerprint(), resolver));
  ...
  public String toString() {
    return name
        + "("
        + arguments.stream().map(OperationArgument::getName).collect(Collectors.joining(","))
        + ")";
  }
  ...
  private static class UnbatchedOperation extends Operation {
    ...
    private UnbatchedOperation(Operation operation) {
      super(
          operation.name,
          unbatchJavaType(operation.typedElement.getJavaType()),
          unbatchContextType(operation.contextType),
          operation.arguments,
          new ArrayList<>(operation.getResolvers()),
