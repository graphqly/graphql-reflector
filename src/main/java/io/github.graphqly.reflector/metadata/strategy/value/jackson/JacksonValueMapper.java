...
import io.github.graphqly.reflector.metadata.strategy.value.*;
...
public class JacksonValueMapper implements ValueMapper, InputFieldBuilder {
  ...
  public <T> T fromInput(Object graphQLInput, Type sourceType, AnnotatedType outputType) {
    try {
      return objectMapper.convertValue(
          graphQLInput, objectMapper.getTypeFactory().constructType(outputType.getType()));
  ...
  public <T> T fromString(String json, AnnotatedType type) {
    ...
    try {
      return objectMapper.readValue(
          json, objectMapper.getTypeFactory().constructType(type.getType()));
    } catch (IOException e) {
  ...
  private Stream<InputField> toInputField(
      AnnotatedType type, BeanPropertyDefinition prop, GlobalEnvironment environment) {
  ...
  private Stream<InputField> toInputField(
    ...
    if (!environment.inclusionStrategy.includeInputField(
        prop.getMutator().getDeclaringClass(), element, element.getJavaType())) {
      return Stream.empty();
    }

    AnnotatedType deserializableType =
        resolveDeserializableType(
            prop.getMutator(), element.getJavaType(), prop.getPrimaryType(), objectMapper);
    Object defaultValue =
        inputInfoGen
            .defaultValue(element.getElements(), element.getJavaType(), environment)
            .orElse(null);
    return Stream.of(
        new InputField(
            prop.getName(),
            prop.getMetadata().getDescription(),
            element,
            deserializableType,
            defaultValue));
  }

  private AnnotatedType resolveDeserializableType(
      Annotated accessor, AnnotatedType realType, JavaType baseType, ObjectMapper objectMapper) {
    ...
    try {
      objectMapper
          ...
          .getFactory()
          .mapAbstractType(
              objectMapper.getDeserializationConfig(), objectMapper.constructType(Map.class));
      JavaType refined =
          introspector.refineDeserializationType(
              objectMapper.getDeserializationConfig(), accessor, baseType);
      ...
      if (!refined.getRawClass().equals(raw)) {
        if (ClassUtils.isSuperClass(realType, refined.getRawClass())) {
          AnnotatedType candidate =
              GenericTypeReflector.getExactSubType(realType, refined.getRawClass());
          if (!ClassUtils.isMissingTypeParameters(candidate.getType())) {
            return candidate;
          }
        }
        return GenericTypeReflector.updateAnnotations(
            TypeUtils.toJavaType(refined), realType.getAnnotations());
      }
    } catch (JsonMappingException e) {
      /*no-op*/
    } catch (Exception e) {
      log.warn(
          "Failed to determine the deserializable type for "
              + GenericTypeReflector.getTypeName(realType.getType())
              + " due to an exception",
          e);
    }
    return realType;
  ...
  protected Object defaultValue(
    ...
    List<TypedElement> annotatedCandidates =
        Utils.flatten(
                Optional.ofNullable(prop.getConstructorParameter())
                    .map(elementFactory::fromConstructorParameter),
  ...
  private static class ElementFactory {
    ...
    TypedElement fromConstructorParameter(AnnotatedParameter ctorParam) {
      ...
      AnnotatedType fieldType =
          transform(
              ClassUtils.getParameterTypes(constructor, type)[ctorParam.getIndex()], parameter);
    ...
    TypedElement fromSetter(AnnotatedMethod setterMethod) {
      ...
      AnnotatedType fieldType =
          transform(ClassUtils.getParameterTypes(setter, type)[0], setter, type);
    ...
    TypedElement fromProperty(BeanPropertyDefinition prop) {
      Optional<TypedElement> constParam =
          Optional.ofNullable(prop.getConstructorParameter()).map(this::fromConstructorParameter);
      ...
      Optional<TypedElement> explicit =
          Utils.flatten(constParam, setter, getter, field)
              .filter(e -> e.isAnnotationPresent(GraphQLInputField.class))
              .findFirst();
