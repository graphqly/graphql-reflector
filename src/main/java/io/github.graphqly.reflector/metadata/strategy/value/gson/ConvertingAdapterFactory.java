...
class ConvertingAdapterFactory implements TypeAdapterFactory {
  ...
  public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
    AnnotatedType detectedType =
        environment.typeTransformer.transform(GenericTypeReflector.annotate(type.getType()));
    return environment.getInputConverters().stream()
        .filter(converter -> converter.supports(detectedType))
        .findFirst()
        .map(
            converter ->
                new ConvertingDeserializer(
                    detectedType,
                    environment.getMappableInputType(detectedType).getType(),
                    converter,
                    environment,
                    gson))
