...
import io.github.graphqly.reflector.annotations.GraphQLComplexity;
...
import io.github.graphqly.reflector.metadata.execution.FieldAccessor;
import io.github.graphqly.reflector.metadata.execution.FixedMethodInvoker;
import io.github.graphqly.reflector.metadata.execution.MethodInvoker;
...
public class PublicResolverBuilder extends AbstractResolverBuilder {
   ...
   *
   * @param javaDeprecationConfig Configures if and how {@code Deprecated} maps to GraphQL
   *     deprecation
   * @return This builder instance to allow chained calls
   */
  public PublicResolverBuilder withJavaDeprecation(
  ...
  public Collection<Resolver> buildQueryResolvers(ResolverBuilderParams params) {
    Set<Property> properties =
        ClassUtils.getProperties(ClassUtils.getRawType(params.getBeanType().getType()));
    Collection<Resolver> propertyAccessors = buildPropertyAccessors(properties.stream(), params);
    Collection<Resolver> methodInvokers =
        buildMethodInvokers(
            params,
            (method, par) ->
                isQuery(method, par)
                    && properties.stream().noneMatch(prop -> prop.getGetter().equals(method)),
            OperationDefinition.Operation.QUERY,
            true);
    Collection<Resolver> fieldAccessors = buildFieldAccessors(params);
    return Utils.concat(
            methodInvokers.stream(), propertyAccessors.stream(), fieldAccessors.stream())
        .collect(Collectors.toSet());
  }

  @Override
  public Collection<Resolver> buildMutationResolvers(ResolverBuilderParams params) {
    return buildMethodInvokers(
        params, this::isMutation, OperationDefinition.Operation.MUTATION, false);
  }

  @Override
  public Collection<Resolver> buildSubscriptionResolvers(ResolverBuilderParams params) {
    return buildMethodInvokers(
        params, this::isSubscription, OperationDefinition.Operation.SUBSCRIPTION, false);
  }

  private Collection<Resolver> buildMethodInvokers(
      ResolverBuilderParams params,
      BiPredicate<Method, ResolverBuilderParams> filter,
    ...
    return Arrays.stream(rawType.getMethods())
        ...
        .filter(
            method ->
                params
                    ...
                    .includeOperation(method, getReturnType(method, params)))
        ...
        .map(
            method -> {
              ...
              OperationInfoGeneratorParams infoParams =
                  new OperationInfoGeneratorParams(
                      element, beanType, querySourceBean, messageBundle, operation);
              return new Resolver(
                  ...
                  messageBundle.interpolate(
                      ReservedStrings.decode(operationInfoGenerator.deprecationReason(infoParams))),
                  batchable && method.isAnnotationPresent(Batched.class),
                  querySourceBean == null
                      ? new MethodInvoker(method, beanType)
                      : new FixedMethodInvoker(querySourceBean, method, beanType),
                  element,
                  argumentBuilder.buildResolverArguments(
                      new ArgumentBuilderParams(
                          method,
                          beanType,
                          params.getInclusionStrategy(),
                          params.getTypeTransformer(),
  ...
  private Collection<Resolver> buildPropertyAccessors(
      Stream<Property> properties, ResolverBuilderParams params) {
    MessageBundle messageBundle = params.getEnvironment().messageBundle;
    AnnotatedType beanType = params.getBeanType();
    Predicate<Member> mergedFilters =
        getFilters().stream().reduce(Predicate::and).orElse(ACCEPT_ALL);
        ...
        .map(
            prop ->
                new AbstractMap.SimpleEntry<>(
                    prop.getGetter(),
                    propertyElementReducer.apply(
                        new TypedElement(getFieldType(prop.getField(), params), prop.getField()),
                        new TypedElement(
                            getReturnType(prop.getGetter(), params), prop.getGetter()))))
        .filter(
            pair ->
                params
                    .getInclusionStrategy()
                    .includeOperation(pair.getValue(), pair.getValue().getJavaType()))
        .map(
            pair -> {
              ...
              OperationInfoGeneratorParams infoParams =
                  new OperationInfoGeneratorParams(
                      element,
                      beanType,
                      params.getQuerySourceBeanSupplier(),
                      messageBundle,
                      OperationDefinition.Operation.QUERY);
              return new Resolver(
                  messageBundle.interpolate(operationInfoGenerator.name(infoParams)),
                  ...
                  params.getQuerySourceBeanSupplier() == null
                      ? new MethodInvoker(getter, beanType)
                      : new FixedMethodInvoker(
                          params.getQuerySourceBeanSupplier(), getter, beanType),
                  element,
                  argumentBuilder.buildResolverArguments(
                      new ArgumentBuilderParams(
                          getter,
                          ...
                          params.getTypeTransformer(),
                          params.getEnvironment())),
                  element.isAnnotationPresent(GraphQLComplexity.class)
                      ? element.getAnnotation(GraphQLComplexity.class).value()
                      : null);
            })
        .collect(Collectors.toSet());
  }
  ...
  private Collection<Resolver> buildFieldAccessors(ResolverBuilderParams params) {
    ...
    return Arrays.stream(ClassUtils.getRawType(beanType.getType()).getFields())
        ...
        .filter(
            field ->
                params.getInclusionStrategy().includeOperation(field, getFieldType(field, params)))
        .map(
            field -> {
  ...
  protected boolean isQuery(Method method, ResolverBuilderParams params) {
    return isPackageAcceptable(method, params)
        && !isMutation(method, params)
        && !isSubscription(method, params);
  }
  ...
  protected boolean isPackageAcceptable(Member method, ResolverBuilderParams params) {
    ...
    basePackages =
        Arrays.stream(basePackages)
            ...
            .toArray(String[]::new); // remove the default package
    return method.getDeclaringClass().equals(beanType)
        || Arrays.stream(basePackages)
            .anyMatch(
                basePackage ->
                    ClassUtils.isSubPackage(method.getDeclaringClass().getPackage(), basePackage));
  }
  ...
  private class DeprecationAwareOperationInfoGenerator extends DefaultOperationInfoGenerator {
    ...
    public String deprecationReason(OperationInfoGeneratorParams params) {
      ...
      if (explicit == null
          && javaDeprecationConfig.enabled
          && params.getElement().isAnnotationPresent(Deprecated.class)) {
