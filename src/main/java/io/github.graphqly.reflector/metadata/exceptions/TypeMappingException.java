package io.github.graphqly.reflector.metadata.exceptions;
...
public class TypeMappingException extends MappingException {
  ...
  public TypeMappingException() {
    super(
        "The provided object is of an unknown type. Provide the type explicitly when registering the bean.");
  }
  ...
  public TypeMappingException(Executable executable, Parameter parameter, Exception cause) {
    super(
        "Parameter \""
            ...
            + "\" is missing generic type parameters and can not be mapped."
            + " For details and possible solutions see "
            + Urls.Errors.AMBIGUOUS_PARAMETER_TYPE,
        cause);
  }
  ...
  public TypeMappingException(Type superType, Type subType) {
    super(
        String.format(
            "Auto discovered type %s can not be uniquely resolved as a subtype of %s",
