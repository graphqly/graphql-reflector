...
/**
 * Class representing a single method used to resolve a specific query given specific arguments. A
 * single query can have multiple resolvers, corresponding to different combinations of arguments.
 * This is done mainly to support attaching multiple overloaded methods as resolvers for the same
 * query. Two resolvers of the same query must not accept the same list of argument names.
 *
...
public class Resolver {
  ...
  private String validateName(String operationName, Executable executable) {
    if (Utils.isEmpty(operationName)) {
      throw new MappingException(
          "The operation name for executable "
              + executable.toString()
  ...
  private void validateBatching(
    ...
    if (contextArguments.isEmpty()
        || !Stream.concat(
                contextArguments.stream().map(arg -> arg.getJavaType().getType()),
                Stream.of(returnType.getType()))
  ...
  /**
   * Finds the argument representing the query context (object returned by the parent query), if it
   * exists. Query context arguments potentially exist only for the resolvers of nestable queries.
   ...
   * @return The arguments representing possible query contexts for this resolver (object returned
   *     by the parent query), or null if this resolver doesn't accept a query context
   */
   ...
   * @throws InvocationTargetException If a reflective invocation of the underlying method/field
   *     fails
   * @throws IllegalAccessException If a reflective invocation of the underlying method/field is not
   *     allowed
   */
  public Object resolve(Object source, Object[] args)
      throws InvocationTargetException, IllegalAccessException {
    return executable.execute(source, args);
  }

  /**
   * Gets the generic Java types of the source objects (object returned by the parent query), if one
   * is accepted by this resolver. Used to decide if this query can be nested inside another.
   *
   * @return The generic Java type of the source object, or null if this resolver does not accept
   *     one.
   */
  public Set<Type> getSourceTypes() {
    return contextArguments.stream()
        .map(arg -> arg.getJavaType().getType())
  ...
  /**
   * Get the fingerprint of this resolver. Fingerprint uniquely identifies a resolver within a
   * query. It is based on the name of the query and all parameters this specific resolver accepts.
  ...
  public boolean equals(Object that) {
    return this == that
        || (that instanceof Resolver && this.executable.equals(((Resolver) that).executable));
  }
